      ...
      {  /*
         ...
         */
         ...
         {  // val doc =  """"<ul>""" + Constitution.constis.map(c => """  <li> Constitution """ + c.id + "</li>").foldLeft("")((a,b) => a + "\n" + b) + "\n</ul>"
            ...
            <tr><td>ID</td><td>description</td></tr>::Constitution.constis.reverse.map(c => <tr><td><a href={ "constitution?id=" + c.id  }>Constitution { c.id }</a></td><td>{ displayNoneIfEmpty(c.shortDescription) }</td></tr>): _*  )
